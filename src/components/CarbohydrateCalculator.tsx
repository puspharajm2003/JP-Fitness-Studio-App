...
import { Apple, Battery, TrendingUp, Activity } from "lucide-react";
...
export function CarbohydrateCalculator() {
  ...
  // Carbohydrate recommendations: 45-65% of total calories
  const carbPercentMin = 45;
  const carbPercentMax = 65;
  const caloriesPerGramCarb = 4;

  const carbGramsMin = useMemo(() => Math.round((tdee * carbPercentMin / 100) / caloriesPerGramCarb), [tdee]);
  const carbGramsMax = useMemo(() => Math.round((tdee * carbPercentMax / 100) / caloriesPerGramCarb), [tdee]);

  const minRecommended = 130; // Institute of Medicine minimum
    ...
    <div className="glass-card rounded-[2rem] p-8 shadow-xl border-none">
      <div className="flex items-center justify-between mb-6">
        <h3 className="font-display font-black text-xl flex items-center gap-3">
          <Apple className="w-5 h-5 text-primary" /> Carb Lab
        </h3>
      ...
      <div className="rounded-[2rem] bg-slate-900 p-8 text-white relative overflow-hidden shadow-2xl">
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-2 mb-4">
             <span className="px-3 py-1 rounded-full bg-white/10 text-[10px] font-black uppercase tracking-widest text-primary">Energy Profile</span>
          </div>
          <div className="flex items-baseline gap-2 mb-2">
            <span className="text-4xl font-black">{carbGramsMin} – {carbGramsMax}</span>
          ...
          <div className="flex items-center gap-3 mt-4 py-3 px-4 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md">
            ...
            <div className="flex flex-col">
              <span className="text-[10px] font-black uppercase tracking-widest text-white/40 leading-none mb-1">Vital Minimum</span>
              <span className="text-xs font-bold">{minRecommended}g Per Day</span>
            </div>
          </div>
          <p className="text-[10px] text-white/40 mt-4 leading-relaxed">
            Targets: {carbPercentMin}–{carbPercentMax}% of {tdee} total calories.
          </p>
        </div>
      </div>
      ...
      <div className="mt-8 pt-8 border-t border-slate-100 dark:border-slate-800">
        ...
        <p className="text-xs text-muted-foreground leading-relaxed">
          Prioritize <span className="text-foreground font-bold">Complex Carbs</span> (oats, legumes, greens) for sustained energy. Minimize blood sugar spikes from refined sugars.
        </p>
      </div>
    </div>
  );
